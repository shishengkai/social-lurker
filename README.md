...
**轻量 R1：0.3.4 已推送并交付，108 项本地测试和 Python 3.12/3.13 CI 通过；实例程序、备份及数据库完整性已独立核验。完整业务与手机推送验收尚未通过，正式日间任务保持暂停。**

| 项目 | 状态 |
| --- | --- |
| 元信息检查、数据库、去重和通知交付协议 | 已实现，本地测试通过 |
| 独立实例安装、重复安装、升级中断恢复 | 已完成原地升级至 0.3.4；新旧版本保留，安装包/备份/数据库及既有通知账本已直接核验 |
| 自动化回归 | 0.3.4 本地 108 项测试、Python 3.12/3.13 GitHub CI 通过 |
| TikHub 真实数据 | 视频号既有作者一页检查通过，另一次分享添加失败；抖音给定链接被接口过滤，完整链路待验收 |
