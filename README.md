...
**轻量 R1：0.3.3 已推送，已直接核验实例程序和数据库完整性；Python 3.12/3.13 CI 通过。0.3.4 已修正视频号错误分类，108 项本地测试通过，待交付。完整业务与手机推送验收尚未通过，正式日间任务保持暂停。**

| 项目 | 状态 |
| --- | --- |
| 元信息检查、数据库、去重和通知交付协议 | 已实现，本地测试通过 |
| 独立实例安装、重复安装、升级中断恢复 | 0.3.1 → 0.3.2 已直接核验；0.3.3 程序摘要及数据库完整性已直接核验，Bot 报告升级前后账本一致 |
| 自动化回归 | 0.3.3 本地 105 项测试、Python 3.12/3.13 GitHub CI 通过 |
| TikHub 真实数据 | 视频号白天一页成功后再次返回 PAGE_IDENTITY_INVALID，已定位为接口仅返回错误提示、缺少作品列表；抖音给定链接被接口过滤，待公开样本 |
| Grok Bot 消息 | 真实回执与长度实测通过；iPhone 两种图片方式均失败，当前使用文字 |
| Grok Bot 后台 | 夜间门禁和原生纯唤醒已实测；正式日间任务保持暂停，完整自动检查与手机强静默尚未通过 |
