...
**轻量 R1：当前实例为 0.3.2；0.3.3 修复抖音空作品提示与单作品响应解析，本地 105 项测试通过，正在交付与真机验收。**

| 项目 | 状态 |
| --- | --- |
| 元信息检查、数据库、去重和通知交付协议 | 已实现，本地测试通过 |
| 独立实例安装、重复安装、升级中断恢复 | 本地验证通过；真实实例已从 0.3.1 原地升级至 0.3.2，旧版、配置和账本保留 |
| 自动化回归 | 0.3.3 本地 105 项测试通过；0.3.2 的 Python 3.12/3.13 GitHub CI 通过 |
| TikHub 真实数据 | 视频号白天重测一页成功；给定抖音链接被接口过滤，公开作品完整验收待完成 |
| Grok Bot 消息 | 真实回执与长度实测通过；iPhone 两种图片方式均失败，当前使用文字 |
| Grok Bot 后台 | 新版夜间门禁已实测；正式日间任务保持暂停，完整自动检查与强静默尚未通过 |
