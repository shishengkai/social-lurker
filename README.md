...
**轻量 R1 / 0.3.1 开发预览：已修复首轮联调问题，本地与视频号接口验证通过，目标 Bot 补丁验收待完成。**

| 项目 | 状态 |
| --- | --- |
| 元信息检查、数据库、去重和通知交付协议 | 已实现，本地测试通过 |
| 独立实例安装、重复安装、升级中断恢复 | 本地验证通过 |
| 自动化回归 | 2026-09-13 补丁验证：82 项测试通过 |
| TikHub 真实数据 | 视频号分享识别、一页列表和原链接已验证；抖音与分页覆盖待验证 |
| Grok Bot 原生调度、静默执行、图文和真实回执 | 旧预览文字试发成功；补丁后的图文、调度与静默待验收 |
| 不可变稳定软件 Release | 尚未发布 |

0.3.1 修复视频号分享详情的有界回退、带图片签名的列表封面、首次图文试发与后台状态登记；消息改为突出标题、紧凑作者/时间及“打开原作品”链接。现有 0.3.0 实例需要受控升级，修改源码不会自动更新 Bot。
