...
**轻量 R1：0.3.1 已在 Grok Bot 实装联调；0.3.2 的后台入口与验证修正已在本地完成，尚未提交或发布。**

| 项目 | 状态 |
| --- | --- |
| 元信息检查、数据库、去重和通知交付协议 | 已实现，本地测试通过 |
| 独立实例安装、重复安装、升级中断恢复 | 本地验证通过 |
| 自动化回归 | 2026-09-13 的 0.3.2 本地验证：96 项测试通过 |
| TikHub 真实数据 | 视频号分享识别、一页列表和原链接已验证；抖音与分页覆盖待验证 |
| Grok Bot 消息 | 0.3.1 真实回执和桌面封面通过；iPhone 两种图片方式均失败，当前使用文字 |
| Grok Bot 后台 | 正式日间任务已纠正并暂停；完整自动检查与强静默尚未通过 |
...
0.3.2 增加固定自动模式的 routine 命令和不调用数据接口的唤醒探针，区分用户选定长度与宿主已验证长度。修改源码不会自动更新 Bot。开发补丁使用新增的**固定 commit 预览升级入口**，由 Bot 保留实例数据完成备份、切换与恢复；`upgrade apply` 仍只选择正式不可变 Release，重复安装不能切换版本。步骤见[安装说明](docs/installation.md)。
...
4. **验证后台运行。** Bot 先用不查作品、不发通知的独立探针核验原生唤醒和静默，再验证允许时段的业务检查；门禁失败就停止，全部通过后才启用日间自动检查。
