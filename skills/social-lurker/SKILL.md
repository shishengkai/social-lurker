...
- `poll`：仅用户前台请求使用；指定作者可加 watch_id。后台例行使用 `routine poll`，不允许自动模式失败后改用前台命令。返回值是内部检查结果，不直接当作对话消息发送。
...
routine 内部先 `routine poll`，再用 `routine next` 按下节处理待发项，两者只接受 `{"protocol":1}`，程序固定执行 automatic 模式。任何能力门禁、夜间限制或接口失败都不能改用前台 poll/dispatch next，不能以“验证”为由绕过。0.3.1 旧实例尚无这两个命令，仍须显式 automatic:true 并遵守相同禁令；不能指示旧实例执行新版本命令。

后台不作版本检查、Star 邀请、不生成“正在检查”“执行完成”等面向用户的过程消息；不能消除宿主固有痕迹时如实报告能力不足。未到允许时间不启动新的例行请求和发送；已经获得的真实回执随时登记。不为余项、重试或版本检查额外唤醒。原生唤醒与强静默的独立验收按维护入口执行，不能拿前台成功替代。
...
图文试发 sent 后，还要实际核对用户阅读端的封面与排版，再由维护入口登记 image_update_id 和 image_evidence；仅“发送工具返回成功”或桌面显示正常不足以证明 iPhone 图片已经显示。2026-09-13 实测视频号封面：原生 images 在 iPhone 成为不可预览文件，Markdown 图片仅显示替代文字；这两种方式在该目标端均未通过。遇到相同情况登记 images_verified:false，后续常规作品使用四字段文字，不改已 sent/unknown 载荷，不补发图片。不得猜测未定义的 filename/MIME 字段、改图片签名或引入下载托管来强行通过。每次试发结束准确列出尚未核验项，不能直接声称后台可用。
