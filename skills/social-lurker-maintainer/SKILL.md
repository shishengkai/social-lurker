...
- sources 键只支持 douyin:normal、douyin:lite、wechat_channels:default；每项含 adapter_version（当前 metadata-r1.3）、level 与 evidence。level 可为 unverified、recent_pages_verified、enumeration_verified、range_verified。分页内容、身份、边界必须有真实接口样本证据；本地 fixture 不可作为线上验证。没有范围/枚举证明，不能说全部作品都已覆盖。版本变化重新验证。
