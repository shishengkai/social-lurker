...
- sources 键只支持 douyin:normal、douyin:lite、wechat_channels:default；每项含 adapter_version（读取当前执行包的 sources.ADAPTER_VERSION，不硬编码示例版本）、level 与 evidence。level 可为 unverified、recent_pages_verified、enumeration_verified、range_verified。分页内容、身份、边界必须有真实接口样本证据；本地 fixture 不可作为线上验证。没有范围/枚举证明，不能说全部作品都已覆盖。版本变化重新验证。
...
业务门禁尚未通过时，先做不涉及业务的宿主核验，不能去掉 automatic 或改用前台扫描“证明”后台可用。0.3.2 的 `routine probe` 接收 `{"protocol":1,"probe_id":"本次唯一标识"}`，只读本实例并返回时间与标识，不调接口、不领通知、不写能力证据，也不自行宣称 silent/verified。调用须使用实际 Python 的 -I -B、实例 run.py 绝对路径及 --instance 绝对路径，再追加 routine probe；核对进程退出码和 JSON envelope.ok，原生任务界面成功不代表命令成功。结果仅供内部核对，不能发给用户作为例行完成消息。
...
`WECHAT_LIST_UNAVAILABLE` 表示视频号在成功信封中只返回错误提示，没有有效作者和作品列表；不能解释为“没有新作品”或断言作者身份变更，也不能据此登记接口已恢复。核对已有参数和供应商状态，不循环重试、不绕过身份校验、不把原始错误或调试信息转发到聊天。`DOUYIN_DETAIL_UNAVAILABLE` 表示抖音未返回可用作品，按安全提示核对或更换公开链接。
