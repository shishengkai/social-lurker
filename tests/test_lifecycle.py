...
def package(tmp_path):
    ...
    descriptor = builder.build(source, output, version="0.3.5", commit="a" * 40)
    raw = (output / "social-lurker-0.3.5.tar.gz").read_bytes()
    descriptor["archive_url"] = (
        "https://github.com/shishengkai/social-lurker/releases/download/v0.3.5/social-lurker-0.3.5.tar.gz"
    )
...
def test_crash_boundaries_restore_pair_or_finish_commit(installed, package, stage):
    ...
    if stage == "committed":
        installed.loaded_version = "0.3.5"
    ...
    assert installed.load()["app_version"] == ("0.3.4" if expect_rollback else "0.3.5")
...
def test_exception_after_durable_commit_never_rolls_back(installed, package):
    ...
    assert installed.load()["app_version"] == "0.3.5"
...
def test_stable_launcher_reenters_target_and_rejects_package_tamper(installed, package):
    ...
    target = installed.path("app/0.3.5/social_lurker/cli.py")
    target.chmod(0o600)
...
def test_package_digest_and_symlink_injection_rejected(installed, package, tmp_path):
    ...
    assert installed.load()["app_version"] == "0.3.4" and installed.plan() is None
